...
struct RandomMoodTile: View {
    var body: some View {
        ZStack {
            // Background
            RoundedRectangle(cornerRadius: 16)
                .fill(Color(hex: "#5F78A3"))
            ...
            Canvas { context, size in
                ...
                for (y, lineWidth) in lines {
                    ...
                    context.fill(line, with: .color(.txtMoodRandom.opacity(0.4)))
                }
                ...
                context.stroke(xPath, with: .color(.txtMoodRandom.opacity(0.05)), lineWidth: 1)

                // Circles
                let circle1 = Path(ellipseIn: CGRect(
                    x: w * 0.56, y: h * 0.024, width: w * 0.33, height: w * 0.33
                ))
                context.fill(circle1, with: .color(.txtMoodRandom.opacity(0.4)))

                let circle2 = Path(ellipseIn: CGRect(
                    x: w * 0.11, y: h * 0.27, width: w * 0.22, height: w * 0.23
                ))
                context.fill(circle2, with: .color(.txtMoodRandom.opacity(0.2)))

                let circle3 = Path(ellipseIn: CGRect(
                    x: w * 0.72, y: h * 0.32, width: w * 0.28, height: w * 0.28
                ))
                context.fill(circle3, with: .color(.txtMoodRandom.opacity(0.3)))
                ...
                context.fill(diamond, with: .color(.txtMoodRandom.opacity(0.8)))
                context.stroke(diamond, with: .color(.txtMoodRandom.opacity(0.2)), lineWidth: 1)
                ...
                for (px, py) in dots {
                    ...
                    context.fill(dot, with: .color(.txtMoodRandom.opacity(0.5)))
                }
            }
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("Random")
                    ...
                    .foregroundStyle(.txtMoodRandom)
                Text("surprise")
                    .font(.system(size: 9))
                    .foregroundStyle(.txtMoodRandom.opacity(0.7))
            }
